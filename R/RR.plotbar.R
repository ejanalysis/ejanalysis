...
#'
#' @description
#'  Draws a plot using group means from \code{\link{RR.means}}
#' @param x Results of \code{\link{RR.means}} function, mean indicator score or risk by demographic group
...
#' @param cex.names optional numeric vector passed to \code{\link{barplot}}
#' @param cex.axis optional numeric vector passed to \code{\link{barplot}}
#' @param ... optional other parameters passed to \code{\link{barplot}}
#' @return draws a barplot, returns the two values as named list
#' @seealso \code{\link{RR.plot}}
#' @examples \dontrun{
...
#'     RR.plotbar(x, 'pctmin', 'proximity.tsdf', dlab = 'Minorities', elab = 'Haz. Waste TSD Facility Proximity Score', ylab = 'Proximity Score')
#'  }
#' @export
RR.plotbar <- function(x, dname, ename, dlab = dname, elab = ename, reflab = paste('Non-', dlab, sep = ''), cex.names = 1.9, cex.axis = 1.5, ...) {
