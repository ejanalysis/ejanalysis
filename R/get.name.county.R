...
# *** ALSO SEE THESE: ****
#
# http://www.census.gov/geo/reference/codes/files/national_county.txt
# get.name.county.R
# get.county.info.R
# GET COUNTY NAMES FROM NHGIS DATASET.R
#############################################################################


#############################################################################
#	EXTRACT TEXT OF COUNTY NAME, AND ALSO STATE (AND TRACT?) NAMES FROM THE NAME FIELD CENSUS PROVIDES
#############################################################################

get.name.county  <- function(placename) {

  # Function to extract partial (e.g., just state or county name) text name of a place from the full Census text name of a place
...
mynames <- get.name.state(testnames)
head(mynames, 40)
