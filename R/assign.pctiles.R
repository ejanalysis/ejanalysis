#' @title Assign percentiles to values (weighted, by zone)
#'
#' @description For each column look at the distribution of values across all rows in a given zone (e.g., places in zone), 
...
#' @return Returns a numeric vector or data.frame same size as x, but if zone is specified, provides percentile with given zone.
#' @template seePctiles
#' @examples
 ...
 if (is.null(zone)) {
  ...
  wtd.Ecdf.results <- wtd.Ecdf(values, weights, type='i/n', na.rm=TRUE) 
   ...
   for (z in unique(zone)) {
     ...
     wtd.Ecdf.results <- wtd.Ecdf(values[zone==z], weights[zone==z], type='i/n', na.rm=TRUE) 
     # If the first CDF estimate is greater than zero, a point (min(x),0) is placed at the beginning of the estimates.
   
     if (length(wtd.Ecdf.results$x) == 1+ length(unique(values[zone==z])) ) { wtd.Ecdf.results$x <- wtd.Ecdf.results$x[-1]; wtd.Ecdf.results$ecdf <- wtd.Ecdf.results$ecdf[-1] }
   
     # This finds the rank of each value
     myindex <- findInterval(values[zone==z], wtd.Ecdf.results$x) 
     myindex[myindex==0] <- 1	# if for some reason the minimum value doesn't match, which has occurred in testing
