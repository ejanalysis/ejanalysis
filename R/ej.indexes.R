...
#' @examples
#' hist(myej <- ej.indexes(data.frame(state.x77[ , c('mortality', '****')]))
#' set.seed(999)
...
ej.indexes <- function(env.df, demog, weights, us.demog, universe.us.demog, as.df=TRUE, prefix="EJ.DISPARITY.", type=1, na.rm=FALSE) {
  ...
  if (missing(us.demog)) {
    ...
    if (missing(universe.us.demog)) {
      ...
      # VERSUS
      #  VSI.US2 <- weighted.mean(VSI.eo, (bg.ftp$pop + bg.ftp$povknownratio)/2)
      ...
      # so it is not quite the correct weighting if you do wtd.mean(VSI.eo, pop)
      
      us.demog <- wtd.mean(demog, weights, na.rm=TRUE) # NOTE THAT  na.rm=TRUE & might make that a parameter of ej.indexes() but unsure when that might be needed
    } else { 
      ...
      us.demog <- wtd.mean(demog, universe.us.demog, na.rm=TRUE) # NOTE THAT  na.rm=TRUE & might make that a parameter of ej.indexes() but unsure when that might be needed
