#' @title Relative Risk by Demographic Group by Indicator based on Census Data
...
#'   # *** note: this currently does not use rrf() & rrfv() but perhaps it would be faster if it did? ***
...
#'                                                                                                                                                                             54464, 97203), temp = c(62.8, 26.6, 60.3, 60.4, 59.4, 45.1, 49,                                                                                                                                                                                                                                       55.3, 70.7, 63.5, 70, 44.4, 51.8, 51.7, 47.8, 54.3, 55.6, 66.4, 
...
#'                                                                                                                                                                                                                                                                                                                                                             -50L))
#' RR(mydat$area, mydat$pcthisp, mydat$pop) # Avg Hispanic lives in a State that is 69 percent larger than that of avg. non-Hispanic
#' RR(mydat$pcthisp, mydat$pcthisp, mydat$pop) # Avg Hispanic lives in a State that has a much higher percent Hispanic than do non-Hispanics
...
RR <- function(e, d, pop, dref, na.rm=TRUE) {
  ...
  RR.for.one.e <- function(e, d, pop, dref) {
    ...
    if (is.vector(d)) {
      # for one e, one d:
      (sum(pop * d *      e, na.rm=na.rm) / sum(pop * d,      na.rm=na.rm)) /
      (sum(pop * (dref) * e, na.rm=na.rm) / sum(pop * (dref), na.rm=na.rm))
    } else {
