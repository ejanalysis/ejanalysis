write.wtd.pctiles.by.zone <- function(mydf, wts, filename, zone.vector) {
  ...
  for (z in unique(zone.vector)) { 
    ...
    r = rbind(r, t(data.frame(mean=sapply(mydf[zone.vector==z,  ], function(x) wtd.mean(x, wts[zone.vector==z], na.rm=TRUE) ) ) ))
    r = rbind(r, t(data.frame(std.dev=sapply(mydf[zone.vector==z,  ], function(x) sqrt(wtd.var(x, wts[zone.vector==z], na.rm=TRUE) ) )) ))
    r
